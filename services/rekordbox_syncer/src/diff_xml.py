...
def drop_by_columns(
    ...
    columns_to_reimport: list,
    columns_to_delete_and_reimport: list,
):
    df_reimport = pd.concat([new_file, old_file], axis=0).drop_duplicates(
        subset=columns_to_reimport, keep=False
    )
    df_delete_reimport = pd.concat([new_file, old_file], axis=0).drop_duplicates(
        subset=columns_to_delete_and_reimport, keep=False
    )
    ...
    df_changed = pd.concat([df_reimport, df_delete_reimport], axis=0).drop_duplicates(
        subset=["@Location"], keep="last"
    )

    df_changed = df_changed.sort_values(by="action", inplace=False).reset_index(
        drop=True
    )

    logger.info(
        f"Found {len(df_changed)} differences (excluding the ones, that are simply new, or not in the specified location of interest"
    )
    files_to_remove = []
...
def main(args):
    ...
    logger.info(
        f"Calculating the difference between new_file: {new_file_path} and old_file: {old_file_path}"
    )
    ...
    columns_to_consider = (
        config["columns_to_consider"] + config["columns_to_consider_and_reimport"]
    )
    new_file = new_file[columns_to_consider]
    old_file = old_file[columns_to_consider]

    newest_date_to_keep = old_file.sort_values(by="@DateAdded", ascending=False)[
        "@DateAdded"
    ].iloc[0]
    ...
    if len_before != len(new_file):
        logger.warning(
            f"Dropped {len_before - len(new_file)} out of {len_before} rows of new file because they are newer than {newest_date_to_keep}"
        )
        new_file.reset_index(drop=True, inplace=True)

    len_before = len(new_file)
    new_file = new_file[
        new_file["@Location"].str.startswith(config["location_to_consider"])
    ]
    if len_before != len(new_file):
        logger.warning(
            f'Dropped {len_before - len(new_file)} out of {len_before} rows of new file because they were not in {config["location_to_consider"]}'
        )
        new_file.reset_index(drop=True, inplace=True)
        assert len(new_file) != 0, logger.error(
            f'The provided location to consider: {config["location_to_consider"]} is not found once in new_file'
        )

    assert len(
        old_file[old_file["@Location"].str.startswith(config["location_to_consider"])]
    ) == len(old_file), logger.error(
        f'Found files with a different location than {config["location_to_consider"]} in old file'
    )
    ...
    drop_by_columns(
        new_file,
        old_file,
        config["columns_to_consider"],
        config["columns_to_consider_and_reimport"],
    )

    # TODO: I don't want to calculate the filename again. Can't i get the logger handler somehow?
    if query_yes_no("Open the log file?"):
        open_file(f'./logs/log {datetime.now().strftime("%Y-%m-%d")}.txt')
