...
def copy_leise_songs_to_directory(xml_file: str, destination_folder: str, keyword: str):
    ...
    location_series = data[data["@Comments"].str.contains(keyword, case=False)][
        "@Location"
    ]
    location_list = location_series.apply(
        lambda x: x[16:]
        .replace("%20", " ")
        .replace("%26", "&")
        .replace("%27", "'")
...
def main(args):
    ...
    if not os.path.exists(args.destination):
        ...
        raise FileExistsError(
            f"Folder {destination_folder} already exists. Aborting..."
        )
